...
import { BookType, Save, User, Table2, CheckCircle2 } from 'lucide-react'
import {
  extractedWords,
  analysisVariables,
  emotions,
...
import type { WordAssignment, WordSelectorRecord } from '@/types/wordSelector.types'
      ...
      <div ref={tabsRef} className="mb-4">
        <div className="flex gap-1 bg-white p-1 rounded-xl shadow overflow-x-auto">
          {analysisVariables.map((variable) => {
            ...
            const varApplicable = Object.values(
              assignmentsByVariable[variable.id]
            ).filter((a) => a.applicable).length
